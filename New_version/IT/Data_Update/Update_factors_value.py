...
def Insert_new_factors_lookup(data):
    try:
        ...
        cutstomer_name = Select_name_customer_from_db(data[6])
        connection = mysql.connector.connect(host="localhost",
                                             user='root',
                                             password='',
                                             database="ERP_IT")
        cursor = connection.cursor()
        mySql_insert_query = """INSERT INTO IT_Factors_lookup (id, customer_id, seller_id, user_id, type, status, total_price, uploaded, seller_name, customer_name) 
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
        x = datetime.datetime.now()
        print(x)
        record = (data[0], data[6], 0, user_id, 'IT', 0, 0, date, 'شرکت اصلی', cutstomer_name)

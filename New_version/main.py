...
def pre_invoice_print_it_ACC():
    if not session.get("Username"):
        ...
        if auth == 1 or auth == 5:
            ...
            invoice_total_tax = "{:,}".format(invoice_total_tax)
            path = session.get('Path')
            return render_template('/ACC/Pre_Invoice/Invoice_Print.html', invoice_total_tax=invoice_total_tax,invoice_total_price_with_tax=invoice_total_price_with_tax,
                                   lookup_factors=lookup_factors, customer_data=customer_data,
                                   invoice_total_price=invoice_total_price, len_code=len(details_factors),
                                   details_factors=details_factors, user=session.get('Username'), pathmain=path,
                                   email=session.get('email'), seller_details=seller_details)
        else:
            return render_template('Not_Permission/index.html')
...
def pre_invoice_print_it_SA():
    if not session.get("Username"):
        ...
        if auth == 4 or auth == 5:
            ...
            invoice_total_tax = "{:,}".format(invoice_total_tax)
            path = session.get('Path')
            return render_template('/ACC/Pre_Invoice/Invoice_Print.html', invoice_total_tax=invoice_total_tax,invoice_total_price_with_tax=invoice_total_price_with_tax, lookup_factors=lookup_factors, customer_data=customer_data, invoice_total_price=invoice_total_price , len_code=len(details_factors), details_factors=details_factors, user=session.get('Username'), pathmain=path, email=session.get('email'), seller_details=seller_details)
        else:
            return render_template('Not_Permission/index.html')
